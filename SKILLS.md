...
- **Two-mode panel-height split via a single `int` toggle.** When a UI region needs an "expand to fill the rest of the panel" mode, the cleanest pattern is a single `int state->foo_expanded` flag plus two height-computation branches: `foo_expanded ? big : small`. Anchor the *complement* (here: chat scroll group) to a small floor (60 px) so the expanded mode still shows context but yields the rest of the screen. The toggle button glyph is a single character (`⤡` U+2921 expand / `⤢` U+2922 collapse, both in DejaVu Sans Mono Geometric Shapes coverage).
