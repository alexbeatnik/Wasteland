...
- **Selected-button styling: flip background AND text colour together.** When a button group represents a single-choice selector (e.g. capability preset OFF / READ / RW / CUSTOM), the active item is normally drawn with the theme's primary colour as its background. In a monochrome amber theme that means the label colour ALSO needs to flip — amber text on an amber background is invisible. The canonical pattern is: snapshot `nk->style.button`, set `normal/hover/active` to `nk_style_item_color(amber)` AND `text_normal/text_hover/text_active` to black, render the active one, then restore. Override all three text states (not just `text_normal`) because Nuklear picks `text_hover` on cursor-over and `text_active` mid-click — missing either gives a one-frame "label vanished" flicker.
...
- **Compact pipeline (summarisation, async):** `ui_compact_chat_history(state, n)` is the only correct call site. v0.7+ replaces the old "drop oldest turn" with an inference-driven summary: snapshot the verbatim tail to keep, queue a summary pass via `inference_request_summary(src_text)`, set `compact_pending = 1`. The main loop polls `inference_take_summary` each frame via `ui_finalize_compact` and splices the result back as `[SUMMARY]\n<body>\n[/SUMMARY]\n<tail>`. Each subsequent compact re-summarises (existing summary + newly-aged turns) so the prefix stays bounded. The SEND button must be gated on `compact_pending` (otherwise a prompt fired during the summary pass would race the post-compact mirror). Direct calls to a hypothetical synchronous compact bypass the async contract — don't add one.
- **Summary block protocol:** `[SUMMARY]\n...\n[/SUMMARY]\n` lives ONLY at byte 0 of `chat_history`. The worker calls `extract_summary_prefix()` first (matches the open marker, scans for a line-starting close marker, copies the body, returns the byte offset past the closing `\n`). The body is appended to the system prompt as `Previous conversation summary: ...`; the remainder of the buffer is handed to `parse_chat_history` unchanged. The `[/SUMMARY]` close-marker scan must be at line start to avoid a false hit on a model output that mentions the literal string mid-text — same defence as the `<think>` tag's line-start guard.
- **Async compact UX:** show "Compacting N older turn(s) into a summary..." while the worker generates; gate SEND; don't render `[SUMMARY]...[/SUMMARY]` specially in the chat panel for v1 (it shows up as raw text — acceptable for now, can be hidden later as a render-time transform like `strip_tool_fences`).
- **Context stats:** compute using `parse_chat_history` + `llama_chat_apply_template` + `llama_tokenize` (probe mode). Call after every generation finish and after every compact. The stats function strips the same `[SUMMARY]` prefix the worker does and prepends a synthetic `system: Summary: ...` message before tokenising — so the CTX bar matches the worker's actual context, including post-compact summaries. Display as `CTX: used / max (pct%)` with colour-coded progress bar (orange > 75 %, red > 90 %).
...
- **Tell the model the budget.** Title and summary prompts both communicate the byte / character budget explicitly ("3 to 5 words, max 40 characters, no quotes, no markdown" for titles; "3-6 sentences, max 600 characters, plain text" for summaries). Without an explicit cap small models pad with filler ("This conversation is about ...") and blow past the on-disk filename limit or the context budget. Pair the prompt-side budget with a hard byte cap in the post-process (filename truncation, summary buffer size) so a model that ignores the instruction still produces something usable.
- **Async secondary inference passes share `run_one_turn`.** Title and summary both reuse the same llama-decode + sample loop as a normal prompt. The mode flags (`title_mode`, `summary_mode`) are checked at the FIRST step of the output emit pipeline (`output_append_locked`) and route bytes into the dedicated buffer before they enter the chat ring buffer. This keeps the chat output untouched during secondary passes — the user doesn't see a half-rendered summary mid-chat.
...
- Authenticated encryption for chat files (`WST2` header + XChaCha20-Poly1305 via Monocypher) — detects tampering and accidental corruption, not nation-state resistance.
...
- **Platform sandbox (`platform_sandbox.c`):** query available capabilities via bitmask (`SANDBOX_CAP_NETWORK_LOCKDOWN`, `SANDBOX_CAP_FS_RESTRICT`, `SANDBOX_CAP_PROCESS_ISOLATE`). On Linux, `platform_sandbox_apply()` installs seccomp-bpf; `platform_sandbox_restrict_fs_to()` installs a Landlock ruleset restricting the process to a single directory. Both are no-ops on macOS/Windows.
- **Filesystem sandbox (`fs_sandbox.c`):** on Linux, use `openat()` traversal with `O_NOFOLLOW` at every step, reject explicit `".."`, and verify `/proc/self/fd/<n>` via `readlink` still lies under the workspace root. On macOS/Windows, fall back to `realpath()` + `path_is_inside()`. This is the **only** place agent tool I/O touches the filesystem.
- **Agent executor subprocess (`agent_executor_main.c`):** on Linux, spawn a separate `wasteland-agent-executor` binary via `posix_spawn`, communicate over a framed Unix socket IPC protocol (`agent_protocol.c`). The executor applies both sandboxes **before** entering its service loop. On non-Linux platforms, IPC is disabled and tools fall back to in-process `fs_sandbox.c` calls. Never `pthread_cancel` the executor — send `AGENT_IPC_TOOL_SHUTDOWN` for graceful termination.
- **Capability presets:** agent mode supports four presets (`OFF`, `READ_ONLY`, `READ_WRITE`, `CUSTOM`) persisted in `wasteland.cfg`. `CUSTOM` stores a per-tool bitmask (`read_file`, `list_dir`, `write_file`, `apply_edit`). The UI renders preset buttons plus per-tool checkboxes when `CUSTOM` is active.
- **Model verification:** background SHA-256 of a `.gguf` file via `verify_compute_sha256()`, driven by a `[ ✓ ]` button in the local vault. Progress is a `volatile int` 0–100; result is shown as green "OK" or red "FAILED".
...
- **Testable-without-SDL rule:** anything that can be tested without an SDL window or a loaded GGUF model should have a suite. Pure string parsers (chat history splitting, system-prompt concatenation, authenticated cipher, HF URL rewrite, chat-name sanitisation), semver comparison, sandbox path resolution, and filesystem-touching tool executors are all ideal candidates.
...
- **Round-trip pattern for authenticated ciphers:** seal + open must recover the exact plaintext. Tamper with one ciphertext byte and assert open fails (authentication check). Use a deterministic nonce in tests; real code uses `crypto_random_bytes()`.
...
- **CI integration:** `cmake --build build && ctest --output-on-failure` runs all suites automatically on every push. Currently 98 tests across 4 suites.
