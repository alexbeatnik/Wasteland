...
- **`state->input_expanded` toggle drives chat/input height split.** When `0`: chat = `height - 200 - pending_panel_h`, input = `34` px. When `1`: chat shrinks to a `60` px sliver and input grows to `height - 220 - pending_panel_h` (with `80` px floor). The toggle button uses U+2921 (`⤡`, expand) / U+2922 (`⤢`, collapse) — both in the embedded DejaVu Sans Mono coverage. Don't replace with ASCII glyphs that look noisy in the amber theme.
